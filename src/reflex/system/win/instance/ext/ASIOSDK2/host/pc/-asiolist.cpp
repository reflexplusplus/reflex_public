#include <windows.h>
#include "../../common/iasiodrv.h"	//REFLEX was #include "iasiodrv.h"
#include "asiolist.h"
...
static LONG findDrvPath (char *clsidstr,char *dllpath,int dllpathsize)
{
	...
	CharLowerBuffA(clsidstr, strlen(clsidstr));	//REFLEX was CharLowerBuff
	if ((cr = RegOpenKeyA(HKEY_CLASSES_ROOT,COM_CLSID,&hkEnum)) == ERROR_SUCCESS) {	//REFLEX was RegOpenKey

		index = 0;
		while (cr == ERROR_SUCCESS && !found) {
			cr = RegEnumKey(hkEnum,index++,(LPTSTR)databuf,512);
			if (cr == ERROR_SUCCESS) {
				CharLowerBuffA(databuf,strlen(databuf));	//REFLEX was CharLowerBuff
...
static LPASIODRVSTRUCT newDrvStruct (HKEY hkey,char *keyname,int drvID,LPASIODRVSTRUCT lpdrv)
{
	...
	if (!lpdrv) {
		if ((cr = RegOpenKeyExA(hkey,keyname,0,KEY_READ,&hksub)) == ERROR_SUCCESS) {	//REFLEX was RegOpenKeyExA(hkey,(LPCTSTR)keyname,0,KEY_READ,&hksub)

			datatype = REG_SZ; datasize = 256;
			cr = RegQueryValueExA(hksub,COM_CLSID,0,&datatype,(LPBYTE)databuf,&datasize);	//REFLEX was RegQueryValueEx
			if (cr == ERROR_SUCCESS) {
				...
				if (rc == 0) {
					...
					if (lpdrv) {
						...
						datatype = REG_SZ; datasize = 256;
						cr = RegQueryValueExA(hksub,ASIODRV_DESC,0,&datatype,(LPBYTE)databuf,&datasize);	//REFLEX was RegQueryValueEx
						if (cr == ERROR_SUCCESS) {
...
AsioDriverList::AsioDriverList ()
{
	...
	cr = RegOpenKeyA(HKEY_LOCAL_MACHINE,ASIO_PATH,&hkEnum);	//REFLEX was RegOpenKey
	while (cr == ERROR_SUCCESS) {
		if ((cr = RegEnumKeyA(hkEnum,index++,keyname,MAXDRVNAMELEN))== ERROR_SUCCESS) {	//REFLEX was RegEnumKey(hkEnum,index++,(LPTSTR)keyname,MAXDRVNAMELEN)
