...
static LONG findDrvPath (char *clsidstr,char *dllpath,int dllpathsize)
{
	...
	CharLowerBuffA(clsidstr,unsigned(strlen(clsidstr)));
	if ((cr = RegOpenKeyA(HKEY_CLASSES_ROOT,COM_CLSID,&hkEnum)) == ERROR_SUCCESS) {
		...
		while (cr == ERROR_SUCCESS && !found) {
			...
			if (cr == ERROR_SUCCESS) {
				CharLowerBuffA(databuf,unsigned(strlen(databuf)));
...
static LPASIODRVSTRUCT newDrvStruct (HKEY hkey,char *keyname,int drvID,LPASIODRVSTRUCT lpdrv)
{
	...
	if (!lpdrv) {
		if ((cr = RegOpenKeyExA(hkey,keyname,0,KEY_READ,&hksub)) == ERROR_SUCCESS) {
			...
			cr = RegQueryValueExA(hksub,COM_CLSID,0,&datatype,(LPBYTE)databuf,&datasize);	//REFLEX was RegOpenKey
			if (cr == ERROR_SUCCESS) {
