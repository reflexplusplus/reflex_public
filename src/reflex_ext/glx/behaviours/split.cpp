...
bool SplitBehaviourImpl::OnPointerTender(Core::PointerAction action, const Core::Pointer & pointer, UInt8 flags, Core::Trap & trap)
{
	if (action > Core::kPointerActionPress)
	{
		return false;
	}
	else if (FindSplitTarget(object, TransformPosition(object, pointer.position)))
	{
...
bool SplitBehaviourImpl::OnEvent(GLX::Object & src, Event & e)
{
	if (IsLeftClick(e))
	{
		...
		if (auto target = FindSplitTarget(object, GetPointerPosition(object, e)))
		{
			...
			EmitTransaction(object, kTransactionStageBegin, g_split_state.idx.value);
	...
	else if (e.id == kMouseDrag)
	{
		if (g_split_state.idx)
		{
			...
			EmitTransaction(object, kTransactionStagePerform, g_split_state.idx.value);
	...
	else if (e.id == kMouseUp)
	{
		if (g_split_state.idx)
		{
			...
			EmitTransaction(object, kTransactionStageEnd, g_split_state.idx.value);
