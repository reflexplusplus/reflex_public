...
struct ContextMenu : public MenuImpl
{
	...
	ContextMenu(Object & src, Key32 context)
		: m_source(src),
		m_oninit(Core::desktop->CreateAnimationClock([this, position = src.GetWindow()->GetMousePosition()](Float)
		{
			m_oninit.Clear();
...
GLX::Object * LookupMouseOver(Object & object)
{
	...
	auto mousepos = GetMousePosition(object);

	if (Contains(rect, mousepos))
	{
		for (auto & i : object) if (Contains(i.GetRect(), mousepos)) return &i;
	}
...
Reflex::TRef <Reflex::GLX::Object> Reflex::GLX::MenuImpl::AddItem(TRef <Object> item)
{
	...
	SetEventDelegate(item, kNullKey, [item](Object & src, Event & e)
	{
		if (e.id == kMouseDown)
		{
			if (auto menu = QueryParentByType<MenuImpl>(src))
			{
				if (auto idx = GLX::LookupBranchIndex(menu->GetContent(), item))
				{
					...
					GLX::Emit(*menu, kMenuSelect, kindex, New<Data::UInt32Property>(idx.value), kitem, item);
