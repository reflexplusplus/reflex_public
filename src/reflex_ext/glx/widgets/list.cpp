...
ReorderContainer::~ReorderContainer()
{
	if (DynamicCast<List>(*m_list))
	{
		...
		if (m_from == m_to)
		{
			...
			EmitRequest(kTransactionStageCancel, m_from, m_to);
		}
		else
		{
			if (EmitRequest(kTransactionStageEnd, m_from, m_to))
			{
...
void ReorderContainer::OnMouseDrag(const Event & e)
{
	auto & list = *m_list;

	Point mousepos = GetPointerPosition(list, e);
	...
	do
	{
		if (EmitRequest(kTransactionStagePerform, m_from, to))
		{
...
bool Reflex::GLX::List::OnEvent(Object & src, Event & e)
{
	if (e.id == kListStartDrag)
	{
		if (CountListSelection(*this) == 1)
		{
			...
			if (EmitReorderRequest(self, kTransactionStageBegin, index, index, false))
			{
