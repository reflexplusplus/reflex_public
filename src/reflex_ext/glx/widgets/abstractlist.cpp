...
bool Reflex::GLX::AbstractList::OnEvent(GLX::Object & src, Event & e)
{
		...
		if (&src != this)
		{
			if (auto branchidx = LookupBranchIndex(*this, src))
			{
				if (auto idx = OnGetIndex(LookupChildAtIndex(*this, branchidx.value)))
				{
					...
					ProcessClick(*this, m_selection_mode, idx.value, flags, GetModifierKeys(e));

					if (!BitCheck(flags, 0)) return true;
				}
			}
		}
	}
	...
	{
		if (g_abstract_list_idx && ExceedsDragThreshold(GetMouseDelta(e), 4.0f))
		{
	...
	else if (e.id == kMouseUp)
	{
		if (g_abstract_list_idx && g_abstract_list_dragging)
		{
			if (CountListSelection(*this) > 1)
			{
				...
				Select(g_abstract_list_idx.value);
			}
			else
			{
				return Emit(MakeIndexedEvent(kListLoad, g_abstract_list_idx.value));
			}
		}
