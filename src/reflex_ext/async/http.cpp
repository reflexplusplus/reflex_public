...
Reflex::TRef <Reflex::Async::Task> Reflex::Async::Detail::CreateHttpRequest(const CString::View & method, const CString::View & url, const HttpHeaders & headers, const Data::Archive::View & body, TRef <HttpRequestCallbacks> callbacks, Detail::NetworkSimulation network_simulation, Output & output)
{
	REFLEX_ASSERT(method && url); //TODO assert on url validation

	output.Log(method, url);

	return Worker::Create([method = CString(method), url = CString(url), headers, body = Data::Archive(body), callbacks = AutoRelease(callbacks), network_simulation, &output](Worker::Context & ctx)
	{
		...
		ctx.SetResult(false);	//so no assert
		...
		default:
			if (callbacks->OnResponse(code))
			{
				...
				ctx.SetResult(true, callbacks->OnComplete());
				break;
			}
			else
			{
