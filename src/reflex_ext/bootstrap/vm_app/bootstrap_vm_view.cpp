...
struct VmViewWrapperImpl : public VmViewWrapper
{
	VmViewWrapperImpl(App & app, const WString::View & path, const ArrayView < Tuple <VM::StaticString, TRef<Reflex::Object>> > & externals, UInt8 flags, const ArrayView <ConstTRef<VM::Module>> & modules)
		...
		m_monitor(CreateScriptObject(path, modules, externals, [this](VM::Context & context, GLX::Object & object)
		{
			...
			{
				for (auto & id : GLX::kBgColourPropertyKeys)
				{
	...
	virtual void OnStoreState(Data::Archive & stream) const override
	{
		...
		if (auto archiveobject_t = VM::GetType<Data::ArchiveObject>(bindings))
		{
			...
			if (onstore)
			{
				...
				auto bytes = Extend(stream, size).data;

				MemCopy(chunk.GetData(), bytes, size);
			}
		}
	}
...
Reflex::TRef <Reflex::Bootstrap::VmViewWrapper> Reflex::Bootstrap::VmViewWrapper::Create(App & app, const WString::View & path, const ArrayView < Tuple <VM::StaticString, TRef<Reflex::Object>> > & externals, UInt8 context_flags, const ArrayView <ConstTRef<VM::Module>> & modules)
{
