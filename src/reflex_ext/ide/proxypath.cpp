...
struct ProxyPathImpl : public ProxyPath
{
	...
	TRef <System::FileHandle> OnRead(const ArrayView <WString::View> & sub_domain, const WString::View & path, File::Attributes & attributes) const override
	{
		if (sub_domain)
		{
	...
	TRef <System::FileHandle> OnWrite(const ArrayView <WString::View> & sub_domain, const WString::View & path, bool append) const override
	{
		if (sub_domain)
		{
