...
BuilderImpl::~BuilderImpl()
{
	//Data::SetBinary(TheGlobal::Get()->m_prefs, K32("IDE:BuilderImpl:File"), Data::Pack(m_content.a));
...
const Map < WString, Array <Address> > & BuilderImpl::GetFilesByGroup()
{
	if (!m_files)
	{
		...
		//REFLEX_RLOOP_PTR(m_state.GetData(), ptr, m_state.GetSize())	//avoid modification assert
		//{
		//	auto key = (*ptr)->key;
