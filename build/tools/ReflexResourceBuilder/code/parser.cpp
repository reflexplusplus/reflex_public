...
private:
	...
	void WriteItem(System::FileHandle & output, Data::Archive & file_scratch, const Array <CString> & ns, Parser::Item & item, const UInt & total, UInt & done, volatile Float & progress);
	...
	{
		auto GetBool = [](const Data::PropertySet & attributes, Key32 id)
		{
			return Key32(Data::GetCString(attributes, id)) == MakeKey32("true");
		};
		...
		item.enumerable = GetBool(attributes, MakeKey32("public"));
	...
	static Key32 HashFileExtension(const WString::View & ext)
	{
		...
		for (auto & i : ext) Reflex::Detail::IncrementHash(hash, Lowercase(i));
	...
	Map < Key32, Function<void(const WString::View & localpath, Array <Item> & items, const Data::PropertySet & attributes)> > m_types;
...
Parser::Parser()
{
	auto file = [this](const WString::View & localpath, Array <Item> & items, const Data::PropertySet & attributes)
	{
		...
		WString path = ToWString(Data::GetCString(attributes, MakeKey32("path")));
	...
	auto folder = [this](const WString::View & localpath, Array <Item> & items, const Data::PropertySet & attributes)
	{
		constexpr auto Convert = [](const ArrayView <CString::View> &values)
		{
			...
			for (auto & i : values) rtn.Push(ToWString(i));

			return rtn;
		};
			
		auto folder = File::CorrectTrailingStroke(ToWString(Data::GetCString(attributes, MakeKey32("path"))));

		auto path = File::ResolveIncludePath(localpath, folder);
	...
	static constexpr auto ReadValue = [](Parser * self, const WString::View & localpath, Array <Item> & items, const Data::PropertySet & attributes)
	{
	...
	{
		auto [item, value] = ReadValue(this, localpath, items, attributes);
...
Pair <WString> Parser::GetOutputPaths(const WString::View & xml_location, const Data::PropertySet & xml)
{
	constexpr auto MakePath = [](const WString::View & xml_location, const CString::View & relative_path, const WString::View & ext) -> WString
	{
		if (relative_path)
		{
			return File::ResolveIncludePath(xml_location, File::CorrectExtension(ToWString(relative_path), ext));
		}

		return {};
	};

	if (auto cpp_path = MakePath(xml_location, Data::GetCString(xml, MakeKey32("output")), L"cpp"))
	{
	...
	else
	{
		return 
		{ 
			MakePath(xml_location, Data::GetCString(xml, MakeKey32("header")), L"h"), 
			MakePath(xml_location, Data::GetCString(xml, MakeKey32("source")), L"cpp") 
		};
	}
}

void Parser::Compile(const WString::View & path, volatile Float & progress)
{
	progress = 0.0f;
	...
	if (h_path && cpp_path)
	{
		...
		Recurse(xml_location, m_root, xml);	//TODO count here


		//build and compare hash
		
		auto start_time = System::GetElapsedTime();
		
		auto dependency_hash = ComputeHash(path);
	
		if (ReadCacheHash(cache_path) == dependency_hash)
		{
			progress = 1.0f;
		...
		for (auto & i : Namespace::BranchIterator(m_root))
		{
			total += i.items.GetSize();
		}
		...
		if (m_format == kFormatReflex)
		{
			...
			File::WriteLine(h, Join(kInclude, ' ', kDoubleQuote, "reflex_ext/file/embedded_resource.h", kDoubleQuote));
		}
		else if (m_format == kFormatReflexMinimal)
		{
			File::WriteLine(h);

			File::WriteLine(h, Join(kInclude, ' ', kDoubleQuote, "reflex/core/array/view.h", kDoubleQuote));
		}
		...
		File::WriteLine(cpp, Join(kInclude, ' ', kDoubleQuote, ToCString(include_path), kDoubleQuote));
		...
		for (auto & group : Namespace::BranchIterator(m_root))
		{
			...
			{
				WriteItem(cpp, file_scratch, ns, i, total, done, progress);
			}
		}
...
UInt ResourceBuilder::Parser::GetLevel(const Namespace & node)
{
	UInt idx = -1;
...
void ResourceBuilder::Parser::Recurse(const WString::View & localpath, Namespace & parent, const Data::PropertySet & attributes)
{
...
void ResourceBuilder::Parser::DeclareNamespace(System::FileHandle & outstream, const Namespace & group)
{
	CString indent;
	...
	if (group.items)
	{
		...
		for (auto & item : group.items)
		{
			...
			//else
			//{
			//	File::WriteLine(outstream, Join(indent, kExtern, kConst, item.enumerable ? kPublicItem : kPrivateItem, kPointer, varname, kSemiColon));
			//}
...
Tuple <UInt,UInt,Data::Archive::View> ResourceBuilder::Parser::UnpackData(Item & item, Data::Archive & scratch)
{
	if (item.source_path)
	{
	...
	while (Modulo(scratch.GetSize(), kSizeOf<UInt64>))
	{
		scratch.Push(0);
	}
...
void ResourceBuilder::Parser::WriteItem(System::FileHandle & file, Data::Archive & file_scratch, const Array <CString> &nspc, Parser::Item & item, const UInt & total, UInt & done, volatile Float & progress)
{
	CString::View sqr_bracket_open("[");

	CString::View beginarray("] =");
	...
	CString data_varname = MakeVarName(item.id);

	CString full_symbol = Join('k', Merge(nspc, '_'), '_');

	File::WriteLine(file, Join(kConst, ' ', kReflex, kUInt64, ' ', full_symbol, data_varname, sqr_bracket_open, ToCString(nblock), beginarray));
	...
	CString archive = Join("reinterpret_cast<const Reflex::UInt8*>(&", full_symbol, data_varname, "), ", ToCString(size), kUL);
		
	CString compressed = Join(ToCString(item.compress ? uncompressed_size : UInt(0)), kUL);

	CString full_namespace = Merge(nspc, kDoubleColon);
...
void ResourceBuilder::Parser::PrintGap(System::FileHandle & file)
{
...
void ResourceBuilder::Parser::PrintComment(System::FileHandle & file, const CString & text)
{
...
CString ResourceBuilder::Parser::MakeHex(const Data::Archive::View & data)
{
	constexpr CString::View kHexPrefix = "0x";

	Data::Archive temp = data;

	auto dst = temp.GetData();

	REFLEX_RLOOP_PTR(data.data, p, data.size)
	{
		(*dst++) = *p;
	}

	return Join(kHexPrefix, Data::BytesToHex(temp));
}

UInt64 ResourceBuilder::Parser::ComputeHash(const WString::View & xml_path)
{
...
UInt64 ResourceBuilder::Parser::ReadCacheHash(const WString & cache_path)
{
	auto handle = Make<System::FileHandle>(cache_path);
	
	UInt64 hash = 0;

	if (handle->Read(&hash, 8) == 8)
	{
		return hash;
	}
...
void ResourceBuilder::Compile(const WString::View & filename, Float & progress)
{
	Parser parser;
